...
class AppGUI(
    ...
    def _build_users_tab(self):
        ...
        ctrl2 = ttk.Frame(left_lf); ctrl2.pack(fill="x", pady=(0,6))
        ttk.Button(ctrl2, text="⚙️ توليد حسابات المعلمين",
                   command=self._teachers_generate_accounts).pack(side="right", padx=3)
        ttk.Button(ctrl2, text="📲 إرسال الحسابات واتساب",
                   command=self._teachers_send_credentials).pack(side="right", padx=3)
        ttk.Button(ctrl2, text="👨‍🏫 إدارة حسابات المعلمين",
                   command=self._teachers_manage_window).pack(side="right", padx=3)
    ...
    # ══════════════════════════════════════════════════════════
    # إدارة حسابات المعلمين — توليد / إرسال / إدارة
    # ══════════════════════════════════════════════════════════

    def _teachers_generate_accounts(self):
        """توليد حسابات (اسم مستخدم وكلمة مرور) لجميع المعلمين بدون إرسال."""
        if not messagebox.askyesno("تأكيد", "سيتم توليد حسابات لجميع المعلمين الذين ليس لديهم حساب بعد.\nهل تريد المتابعة؟"):
            return
        import random
        ...
        for t in teachers_data:
            ...
            if ok:
                save_user_allowed_tabs(username, ROLE_TABS.get("teacher", []))
                generated.append({"name": name, "username": username, "password": password, "phone": phone})
                success_count += 1

        self._users_load()
        # احفظ كلمات المرور المولّدة مؤقتاً لإرسالها لاحقاً
        self._last_generated_teachers = generated
        messagebox.showinfo("اكتمل",
            f"✅ تم إنشاء {success_count} حساب معلم.\n"
            f"⏭ تم تخطي {skip_count} (موجود مسبقاً أو بياناته ناقصة).\n\n"
            f"يمكنك الآن الضغط على 'إرسال الحسابات واتساب' لإرسال بيانات الدخول.")

    def _teachers_send_credentials(self):
        """نافذة اختيار المعلمين لإرسال بيانات الدخول عبر واتساب."""
        generated = getattr(self, "_last_generated_teachers", [])
        if not generated:
            messagebox.showinfo("تنبيه",
                "لا توجد حسابات مولّدة حديثاً للإرسال.\n"
                "اضغط 'توليد حسابات المعلمين' أولاً لتوليد حسابات جديدة.")
            return

        login_url = "https://darbte.uk/web/login"

        # ── نافذة اختيار المعلمين ──
        win = tk.Toplevel(self.root)
        win.title("📲 إرسال بيانات الدخول")
        win.geometry("550x500")
        win.transient(self.root); win.grab_set()

        hdr = tk.Frame(win, bg="#2E7D32", height=42)
        hdr.pack(fill="x"); hdr.pack_propagate(False)
        tk.Label(hdr, text="📲 اختر المعلمين لإرسال بياناتهم",
                 bg="#2E7D32", fg="white",
                 font=("Tahoma",11,"bold")).pack(side="right", padx=14, pady=8)
        ...
        def select_all():
            for v in check_vars: v[1].set(True)
        def deselect_all():
            for v in check_vars: v[1].set(False)

        ttk.Button(ctrl, text="✅ تحديد الكل", command=select_all).pack(side="right", padx=4)
        ttk.Button(ctrl, text="☐ إلغاء الكل", command=deselect_all).pack(side="right", padx=4)
        ttk.Label(ctrl, text=f"عدد المعلمين: {len(generated)}",
                  font=("Tahoma",9), foreground="#5A6A7E").pack(side="left", padx=4)

        # قائمة المعلمين مع checkboxes
        list_canvas = tk.Canvas(win, highlightthickness=0)
        list_sb = ttk.Scrollbar(win, orient="vertical", command=list_canvas.yview)
        list_inner = ttk.Frame(list_canvas)

        def on_list_conf(e):
            list_canvas.configure(scrollregion=list_canvas.bbox("all"))
        list_inner.bind("<Configure>", on_list_conf)
        ...
        for idx, t in enumerate(generated):
            ...
            phone_info = t.get("phone","") or "بدون رقم"
            cb = ttk.Checkbutton(list_inner,
                text=f"{t['name']}  —  {phone_info}",
                variable=var)
            cb.grid(row=idx, column=0, sticky="w", padx=10, pady=3)
            check_vars.append((t, var))

        # زر الإرسال
        def do_send():
            selected = [(t, v) for t, v in check_vars if v.get()]
            if not selected:
                messagebox.showwarning("تنبيه", "لم تحدد أي معلم للإرسال.", parent=win)
                return
            if not messagebox.askyesno("تأكيد",
                f"إرسال بيانات الدخول لـ {len(selected)} معلم؟", parent=win):
                return
            ...
            for t, _ in selected:
                ...
                if not phone:
                    failed += 1; continue
                msg = (f"مرحباً أستاذ {t['name']}\n\n"
                       f"يسعدنا انضمامك لنظام الغياب الإلكتروني.\n"
                       f"بيانات الدخول الخاصة بك:\n\n"
                       f"🔗 الرابط:\n{login_url}\n\n"
                       f"👤 اسم المستخدم: {t['username']}\n"
                       f"🔑 كلمة المرور: {t['password']}\n\n"
            ...
            # احذف المُرسَلين من القائمة المؤقتة
            sent_usernames = {t["username"] for t, v in selected if v.get()}
            self._last_generated_teachers = [
                t for t in self._last_generated_teachers
                if t["username"] not in sent_usernames]

            messagebox.showinfo("اكتمل",
                f"✅ تم إرسال {sent} رسالة بنجاح.\n"
                f"❌ فشل إرسال {failed} رسالة.", parent=win)
            win.destroy()

        ttk.Button(win, text="📨 إرسال للمحددين", command=do_send).pack(pady=10)

    def _teachers_manage_window(self):
        """نافذة إدارة شاملة لحسابات المعلمين — تجديد كلمة المرور وإظهار/إخفاء التبويبات."""
        import json as _j

        win = tk.Toplevel(self.root)
        win.title("👨‍🏫 إدارة حسابات المعلمين")
        ...
        hdr.pack(fill="x"); hdr.pack_propagate(False)
        tk.Label(hdr, text="👨‍🏫 إدارة حسابات المعلمين",
                 bg="#1565C0", fg="white",
                 font=("Tahoma",12,"bold")).pack(side="right", padx=14, pady=10)

        # ── تقسيم: القائمة + التفاصيل ──
        paned = ttk.PanedWindow(win, orient="horizontal")
        paned.pack(fill="both", expand=True, padx=8, pady=8)

        # ── الجانب الأيمن: قائمة المعلمين ──
        list_lf = ttk.LabelFrame(paned, text=" قائمة المعلمين ", padding=6)
        paned.add(list_lf, weight=2)

        cols_t = ("username","full_name","active")
        tree_t = ttk.Treeview(list_lf, columns=cols_t, show="headings", height=20)
        for c, h, w in zip(cols_t, ["اسم المستخدم","الاسم الكامل","الحالة"], [140,200,70]):
            tree_t.heading(c, text=h)
            tree_t.column(c, width=w, anchor="center")
        tree_t.tag_configure("inactive", foreground="#9E9E9E")
        sb_t = ttk.Scrollbar(list_lf, orient="vertical", command=tree_t.yview)
        ...
        detail_lf = ttk.LabelFrame(paned, text=" تفاصيل المعلم ", padding=10)
        paned.add(detail_lf, weight=3)

        info_lbl = ttk.Label(detail_lf, text="← اختر معلماً من القائمة",
                             font=("Tahoma",11,"bold"), foreground="#5A6A7E")
        info_lbl.pack(pady=(4,12))

        # ── أزرار التحكم ──
        btns_frame = ttk.Frame(detail_lf)
        btns_frame.pack(fill="x", pady=(0,10))

        selected_teacher = {"username": None}

        def reset_pw():
            un = selected_teacher["username"]
            if not un:
                messagebox.showwarning("تنبيه", "حدد معلماً أولاً", parent=win)
                return
            import random
            new_pw = str(random.randint(100000, 999999))
            update_user_password(un, new_pw)
            messagebox.showinfo("تم",
                f"✅ تم تجديد كلمة المرور:\n\n"
                f"اسم المستخدم: {un}\n"
                f"كلمة المرور الجديدة: {new_pw}\n\n"
                f"يمكنك نسخها وإرسالها للمعلم.",
                parent=win)
        ...
        def toggle_active():
            ...
            if not un: return
            import sqlite3 as _sq
            con = _sq.connect(DB_PATH); con.row_factory = _sq.Row
            cur = con.cursor()
            cur.execute("SELECT id, active FROM users WHERE username=?", (un,))
            row = cur.fetchone(); con.close()
            if not row: return
            toggle_user_active(row["id"], 0 if row["active"] else 1)
            load_teachers_list()
            self._users_load()
        ...
        def send_individual():
            ...
            if not un:
                messagebox.showwarning("تنبيه", "حدد معلماً أولاً", parent=win)
                return
            import random, sqlite3 as _sq2
            # ابحث عن رقم جوال المعلم
            teachers_data = load_teachers().get("teachers", [])
            con2 = _sq2.connect(DB_PATH); con2.row_factory = _sq2.Row
            cur2 = con2.cursor()
            cur2.execute("SELECT full_name FROM users WHERE username=?", (un,))
            urow = cur2.fetchone(); con2.close()
            full_name = urow["full_name"] if urow else un

            phone = ""
            for t in teachers_data:
                tid = t.get("رقم الهوية", "").strip()
                tph = t.get("رقم الجوال", "").strip()
                tname = t.get("اسم المعلم", "").strip()
                if tid == un or tph == un or tname == full_name:
                    phone = tph
                    break

            if not phone:
                phone = simpledialog.askstring("رقم الجوال",
                    f"أدخل رقم جوال المعلم '{full_name}':",
                    parent=win)
                if not phone: return
            ...
            msg = (f"مرحباً أستاذ {full_name}\n\n"
                   f"بيانات الدخول المحدّثة:\n\n"
                   f"🔗 الرابط:\n{login_url}\n\n"
                   f"👤 اسم المستخدم: {un}\n"
                   f"🔑 كلمة المرور: {new_pw}\n\n"
                   f"مع تحيات إدارة المدرسة")
            try:
                send_whatsapp_message(phone, msg)
                messagebox.showinfo("تم",
                    f"✅ تم تجديد كلمة المرور وإرسالها للمعلم '{full_name}' على الرقم {phone}.",
                    parent=win)
            except Exception as ex:
                messagebox.showerror("خطأ", f"❌ فشل الإرسال: {ex}", parent=win)

        ttk.Button(btns_frame, text="🔑 تجديد كلمة المرور",
                   command=reset_pw).pack(side="right", padx=4)
        ttk.Button(btns_frame, text="📲 تجديد وإرسال واتساب",
                   command=send_individual).pack(side="right", padx=4)
        ttk.Button(btns_frame, text="🔄 تفعيل/تعطيل",
                   command=toggle_active).pack(side="right", padx=4)

        ttk.Separator(detail_lf, orient="horizontal").pack(fill="x", pady=(0,8))

        # ── صلاحيات التبويبات ──
        ttk.Label(detail_lf, text="التبويبات المسموحة:",
                  font=("Tahoma",10,"bold")).pack(anchor="e", pady=(0,4))

        tabs_quick = ttk.Frame(detail_lf)
        tabs_quick.pack(fill="x", pady=(0,6))

        def select_all_tabs():
            for v in tab_vars.values(): v.set(True)
        def deselect_all_tabs():
            for v in tab_vars.values(): v.set(False)
        def reset_defaults():
            defaults = set(ROLE_TABS.get("teacher", []))
            for t, v in tab_vars.items(): v.set(t in defaults)

        ttk.Button(tabs_quick, text="تحديد الكل", command=select_all_tabs).pack(side="right", padx=3)
        ttk.Button(tabs_quick, text="إلغاء الكل", command=deselect_all_tabs).pack(side="right", padx=3)
        ttk.Button(tabs_quick, text="افتراضي", command=reset_defaults).pack(side="right", padx=3)

        # ── checkboxes للتبويبات ──
        tabs_canvas = tk.Canvas(detail_lf, highlightthickness=0, height=250)
        tabs_sb = ttk.Scrollbar(detail_lf, orient="vertical", command=tabs_canvas.yview)
        tabs_inner = ttk.Frame(tabs_canvas)

        def on_tabs_conf(e):
            tabs_canvas.configure(scrollregion=tabs_canvas.bbox("all"))
        tabs_inner.bind("<Configure>", on_tabs_conf)
        tabs_canvas.create_window((0,0), window=tabs_inner, anchor="nw")
        tabs_canvas.configure(yscrollcommand=tabs_sb.set)
        tabs_canvas.pack(side="left", fill="both", expand=True)
        tabs_sb.pack(side="right", fill="y")

        teacher_tabs = [
            "لوحة المراقبة", "روابط الفصول", "التأخر", "الأعذار",
            "الاستئذان", "المراقبة الحية", "الموجّه الطلابي", "استلام تحويلات",
            "السجلات / التصدير", "إدارة الغياب", "التقارير / الطباعة",
            "تقرير الفصل", "نشر النتائج", "تحليل النتائج", "تصدير نور",
            "الإشعارات الذكية", "إرسال رسائل الغياب", "رسائل التأخر",
            "مستلمو التأخر", "جدولة الروابط", "إدارة الواتساب",
            "إدارة الطلاب", "إضافة طالب", "إدارة الفصول",
            "إدارة أرقام الجوالات",
            "تحويل طالب", "نماذج المعلم", "خطابات الاستفسار",
            "إعدادات المدرسة", "المستخدمون", "النسخ الاحتياطية",
        ...
        tab_vars = {}
        COLS = 2
        for idx, tab_name in enumerate(teacher_tabs):
            var = tk.BooleanVar(value=False)
            tab_vars[tab_name] = var
            r, c = divmod(idx, COLS)
            ttk.Checkbutton(tabs_inner, text=tab_name, variable=var).grid(
                row=r, column=c, sticky="w", padx=10, pady=3)
        for c in range(COLS):
            tabs_inner.columnconfigure(c, weight=1)
        ...
        def save_tabs():
            ...
            if not un:
                messagebox.showwarning("تنبيه", "حدد معلماً أولاً", parent=win)
                return
            selected = [t for t, v in tab_vars.items() if v.get()]
            save_user_allowed_tabs(un, selected)
            self._users_load()
            messagebox.showinfo("تم", f"✅ تم حفظ {len(selected)} تبويب للمعلم.", parent=win)

        ttk.Button(detail_lf, text="💾 حفظ صلاحيات التبويبات",
                   command=save_tabs).pack(pady=(8,4))

        # ── تحميل قائمة المعلمين ──
        def load_teachers_list():
            for i in tree_t.get_children(): tree_t.delete(i)
            for u in get_all_users():
                if u["role"] != "teacher": continue
                tag = "inactive" if not u["active"] else ""
                tree_t.insert("", "end", tags=(tag,),
                    values=(u["username"], u.get("full_name",""), "✅" if u["active"] else "❌"))

        def on_teacher_select(e=None):
            sel = tree_t.selection()
            if not sel: return
            vals = tree_t.item(sel[0], "values")
            un = vals[0]
            selected_teacher["username"] = un
            info_lbl.configure(text=f"المعلم: {vals[1] or un}", foreground="#1565C0")

            # حمّل التبويبات
            import sqlite3 as _sq
            con = _sq.connect(DB_PATH); con.row_factory = _sq.Row
            cur = con.cursor()
            cur.execute("SELECT allowed_tabs FROM users WHERE username=?", (un,))
            row = cur.fetchone(); con.close()
            if row and row["allowed_tabs"]:
                try:
                    allowed = set(_j.loads(row["allowed_tabs"]))
                except:
                    allowed = set(ROLE_TABS.get("teacher", []))
            else:
                allowed = set(ROLE_TABS.get("teacher", []))
            for t, v in tab_vars.items():
                v.set(t in allowed)

        tree_t.bind("<<TreeviewSelect>>", on_teacher_select)
        load_teachers_list()
